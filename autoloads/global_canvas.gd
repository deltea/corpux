...
func _on_pixelate(duration: float) -> void:
	pixelate.visible = true
	var tween := create_tween().set_trans(Tween.TRANS_QUART).set_ease(Tween.EASE_OUT)
	tween.tween_property(pixelate.material, "shader_parameter/pixelSize", 1000, duration)
	tween.tween_callback(func() -> void: pixelate.visible = false)


func _on_unpixelate(duration: float) -> void:
	pixelate.visible = true
...
func set_smear(amount: float) -> void:
	crt.material.set_shader_parameter("luma_smear_px", BASE_SMEAR + amount)


func _on_flashbang(duration: float, amount: float = 1.0) -> void:
